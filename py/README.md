# Sharedmobilitych Python SDK

The Python SDK for the Sharedmobilitych API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Asset(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Sharedmobilitych(None).load(
    {"id": "test01"}, None
)
...
```
py/
├── sharedmobilitych_sdk.py         -- Main SDK module

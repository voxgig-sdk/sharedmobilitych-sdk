...
class TestAttributeDirect:
    ...
    def test_should_direct_list_attribute(self):
        setup = _attribute_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-attribute", "live" if setup["live"] else "unit")
        if _skip:
...
def _attribute_direct_setup(mockres):

...
class TestRegionDirect:

    def test_should_direct_list_region(self):
        setup = _region_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-region", "live" if setup["live"] else "unit")
        if _skip:
...
def _region_direct_setup(mockres):

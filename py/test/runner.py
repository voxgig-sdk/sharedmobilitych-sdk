# Sharedmobilitych SDK test runner
...
class SharedmobilitychTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            SharedmobilitychTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = SharedmobilitychTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = SharedmobilitychTestRunner.getenv("SHAREDMOBILITYCH_TEST_LIVE")
        override = SharedmobilitychTestRunner.getenv("SHAREDMOBILITYCH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = SharedmobilitychTestRunner.getenv(key)
        ...
        explain = SharedmobilitychTestRunner.getenv("SHAREDMOBILITYCH_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SHAREDMOBILITYCH_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if SharedmobilitychTestRunner._test_control is not None:
            return SharedmobilitychTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                SharedmobilitychTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            SharedmobilitychTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = SharedmobilitychTestRunner.load_test_control()
...
def load_env_local():
    SharedmobilitychTestRunner.load_env_local()


def env_override(m):
    return SharedmobilitychTestRunner.env_override(m)


def entity_data(v):
    return SharedmobilitychTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return SharedmobilitychTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return SharedmobilitychTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return SharedmobilitychTestRunner.load_test_control()


def live_delay_ms():
    return SharedmobilitychTestRunner.live_delay_ms()

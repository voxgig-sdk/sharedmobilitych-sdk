...
from sharedmobilitych_sdk.sharedmobilitych_types import (
    Region,
    RegionListMatch,
)
...
class RegionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RegionEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Region]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Region().list() works with no args.
        if reqmatch is None:
            reqmatch = {}

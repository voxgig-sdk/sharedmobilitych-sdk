# sharedmobilitych-cli

boru-driven command-line client **and** interactive REPL for the Sharedmobilitych
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sharedmobilitych-cli)
make build

# 2. See usage (words, entities, env vars)
./sharedmobilitych-cli --help

# 3. Provide credentials once, via the environment
export SHAREDMOBILITYCH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./sharedmobilitych-cli load 1 asset            # {id:1} shorthand
./sharedmobilitych-cli load '{id:1}' asset       # explicit match map

# 5. Override the API base URL for a single call
SHAREDMOBILITYCH_BASE=https://api.example.com ./sharedmobilitych-cli load 1 asset

# 6. No arguments -> interactive REPL
./sharedmobilitych-cli
sharedmobilitych> load 1 asset
sharedmobilitych> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sharedmobilitych-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/sharedmobilitych-cli load 1 asset
   ```
...
```sh
./sharedmobilitych-cli load 1 asset          # scalar shorthand for {id:1}
./sharedmobilitych-cli load '{id:1}' asset     # explicit match map
```
...
./sharedmobilitych-cli load 1 asset
```
...
Run with no arguments to open a REPL (prompt `sharedmobilitych>`). Each line is
evaluated as its own boru expression:

```text
$ ./sharedmobilitych-cli
sharedmobilitych> load 1 asset
sharedmobilitych> /help
sharedmobilitych> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `asset`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/sharedmobilitych-cli`. |
...
(`./sharedmobilitych-cli <expr>`) and interactively (the REPL), and expressions compose the

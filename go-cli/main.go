// Command Sharedmobilitych-cli is a CLI + REPL for the Sharedmobilitych SDK,
...
const prompt = "sharedmobilitych"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "asset attribute provider region search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SHAREDMOBILITYCH_APIKEY (api key), SHAREDMOBILITYCH_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SHAREDMOBILITYCH_APIKEY carries the API key and
	// SHAREDMOBILITYCH_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SHAREDMOBILITYCH_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SHAREDMOBILITYCH_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewSharedmobilitychSDK(opts)

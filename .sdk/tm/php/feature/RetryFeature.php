...
// Sharedmobilitych SDK retry feature
...
class SharedmobilitychRetryFeature extends SharedmobilitychBaseFeature
{
    ...
    public function init(SharedmobilitychContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (SharedmobilitychContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(SharedmobilitychContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(SharedmobilitychContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

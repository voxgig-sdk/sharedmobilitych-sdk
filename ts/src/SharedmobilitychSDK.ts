...
import { SharedmobilitychEntityBase } from './SharedmobilitychEntityBase'
...
class SharedmobilitychSDK {
  ...
  async direct(fetchargs?: any) {
    if (!this._options.allow.op.includes('direct')) {
      return {
        ok: false,
        err: new Error('SharedmobilitychSDK: direct: operation not allowed by' +
  ...
  async graphql(query: string, variables?: any, ctrl?: any) {
    ...
    if (!options.allow.op.includes('graphql')) {
      return {
        ok: false,
        err: new Error('SharedmobilitychSDK: graphql: operation not allowed by' +
    ...
    if (null != errors && Array.isArray(errors) && 0 < errors.length) {
      ...
      const err: any = new Error('SharedmobilitychSDK: graphql: ' +
  ...
  static test(testoptsarg?: any, sdkoptsarg?: any) {
    ...
    const testsdk = new SharedmobilitychSDK(sdkopts)
    testsdk._mode = 'test'
  ...
  tester(testopts?: any, sdkopts?: any) {
    return SharedmobilitychSDK.test(testopts, sdkopts)
  }


  toJSON() {
    return { name: 'Sharedmobilitych' }
  }

  toString() {
    return 'Sharedmobilitych ' + this._utility.struct.jsonify(this.toJSON())
  }

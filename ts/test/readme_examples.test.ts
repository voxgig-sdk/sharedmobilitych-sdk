...
//     (\`new SharedmobilitychSDK(...)\` / \`SharedmobilitychSDK.test(...)\` become
//     \`SharedmobilitychSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'SharedmobilitychSDK'
...
const TEST_SEED = {"entity":{"asset":{"test01":{"id":"test01"}},"attribute":{"test01":{"id":"test01"}},"provider":{"test01":{"id":"test01"}},"region":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new SharedmobilitychSDK(...)\` and any
// \`SharedmobilitychSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+SharedmobilitychSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/SharedmobilitychSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

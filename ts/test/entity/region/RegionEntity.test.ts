...
describe('RegionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'region.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['SHAREDMOBILITYCH_TEST_REGION_ENTID']
  ...
  idmap = env['SHAREDMOBILITYCH_TEST_REGION_ENTID']

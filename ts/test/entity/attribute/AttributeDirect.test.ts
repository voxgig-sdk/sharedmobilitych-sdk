...
import { SharedmobilitychSDK } from '../../..'
...
describe('AttributeDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new SharedmobilitychSDK({
      system: { fetch: async () => ({}) }
    })
  ...
  test('direct-list-attribute', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-attribute', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHAREDMOBILITYCH_TEST_ATTRIBUTE_ENTID']

...
class RegionDirectTest < Minitest::Test
  def test_direct_list_region
    setup = region_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-region", setup[:live] ? "live" : "unit")
    if _should_skip
...
def region_direct_setup(mockres)

...
class ProviderEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Provider(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Provider(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHAREDMOBILITYCH_TEST_PROVIDER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    provider_ref01_data = nil
    if provider_ref01_data_raw.length > 0
    ...
    provider_ref01_list_result = provider_ref01_ent.list(provider_ref01_match, nil)
    assert provider_ref01_list_result.is_a?(Array)
...
def provider_basic_setup(extra)
  ...
  entid_env_raw = ENV["SHAREDMOBILITYCH_TEST_PROVIDER_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["SHAREDMOBILITYCH_TEST_PROVIDER_ENTID"])

...
class ProviderDirectTest < Minitest::Test
  def test_direct_list_provider
    setup = provider_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-provider", setup[:live] ? "live" : "unit")
    if _should_skip
...
def provider_direct_setup(mockres)
  Runner.load_env_local

  calls = []

  env = Runner.env_override({
    "SHAREDMOBILITYCH_TEST_PROVIDER_ENTID" => {},

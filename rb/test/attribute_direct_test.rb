...
class AttributeDirectTest < Minitest::Test
  def test_direct_list_attribute
    setup = attribute_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-attribute", setup[:live] ? "live" : "unit")
    if _should_skip
...
def attribute_direct_setup(mockres)
  ...
  if live
    ...
    client = SharedmobilitychSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,

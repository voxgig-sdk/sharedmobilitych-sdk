...
module SharedmobilitychUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = SharedmobilitychResponse.new({})
    ...
    if fetch_err
      ...
      response = SharedmobilitychResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = SharedmobilitychResponse.new(fetched)
    else

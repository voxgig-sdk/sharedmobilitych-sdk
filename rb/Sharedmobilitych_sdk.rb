...
require_relative 'Sharedmobilitych_types'


class SharedmobilitychSDK
  ...
  def initialize(options = {})
    ...
    utility = SharedmobilitychUtility.new
    @_utility = utility
    ...
    feature_opts = SharedmobilitychHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = SharedmobilitychHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, SharedmobilitychFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    SharedmobilitychUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = SharedmobilitychHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = SharedmobilitychHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = SharedmobilitychHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = SharedmobilitychSpec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => SharedmobilitychError.new(
        "#{op}_allow",
        "SharedmobilitychSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue SharedmobilitychError => err
    ...
    if fetched.is_a?(Hash)
      status = SharedmobilitychHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = SharedmobilitychError.new(
        "graphql_error", "SharedmobilitychSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = SharedmobilitychSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

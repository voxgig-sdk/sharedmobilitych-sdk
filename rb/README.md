# Sharedmobilitych Ruby SDK

The Ruby SDK for the Sharedmobilitych API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Asset(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.Sharedmobilitych(nil).load(
  { "id" => "test01" }, nil
)
...
```ruby
require_relative "Sharedmobilitych_sdk"
client = SharedmobilitychSDK.new(options)
```
...
```ruby
client = SharedmobilitychSDK.test(testopts, sdkopts)
```
...
```
rb/
├── Sharedmobilitych_sdk.rb       -- Main SDK module
...
The main module (`Sharedmobilitych_sdk`) exports the SDK class

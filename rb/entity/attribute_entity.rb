# Sharedmobilitych SDK Attribute entity
...
class AttributeEntity
  ...
  def make
    opts = @_entopts.dup
    AttributeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = SharedmobilitychHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Attribute, Hash] the current Attribute data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = SharedmobilitychHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Attribute fields)
  def match_get
  ...
  # List Attribute items matching the given filter.
  #
  # @param reqmatch [AttributeListMatch, Hash, nil] match filter (any subset of
  #   Attribute fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Attribute>, Array] the matching Attribute items; raises SharedmobilitychError on failure

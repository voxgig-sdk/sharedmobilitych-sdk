...
// Sharedmobilitych SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class SharedmobilitychTestFeature extends SharedmobilitychBaseFeature
{
    ...
    public function init(SharedmobilitychContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (SharedmobilitychContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = SharedmobilitychParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (SharedmobilitychContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(SharedmobilitychContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = SharedmobilitychParam::call($ctx, $k);

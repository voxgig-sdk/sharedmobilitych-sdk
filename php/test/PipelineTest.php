...
// Sharedmobilitych SDK pipeline test
//
...
{
    private static function utility(): SharedmobilitychUtility
    {
        return new SharedmobilitychUtility();
    }
    ...
    private static function ctx(array $over = []): SharedmobilitychContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new SharedmobilitychContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new SharedmobilitychOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof SharedmobilitychError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new SharedmobilitychOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = SharedmobilitychMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new SharedmobilitychOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = SharedmobilitychMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new SharedmobilitychOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = SharedmobilitychMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SharedmobilitychSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = SharedmobilitychMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new SharedmobilitychResponse([]),
            'result' => new SharedmobilitychResult([]),
        ]);
        [, $err] = SharedmobilitychMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new SharedmobilitychSpec([]),
            'response' => null,
            'result' => new SharedmobilitychResult([]),
        ]);
        [, $err] = SharedmobilitychMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new SharedmobilitychSpec(['step' => 's']),
            'response' => new SharedmobilitychResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new SharedmobilitychResult([]),
        ]);
        [, $err] = SharedmobilitychMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new SharedmobilitychSpec(['step' => 's']),
            'response' => new SharedmobilitychResponse(self::resp(200, ['v' => 1])),
            'result' => new SharedmobilitychResult([]),
        ]);
        [, $err] = SharedmobilitychMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new SharedmobilitychSpec(['step' => 's']),
            'response' => new SharedmobilitychResponse(self::resp(200, ['v' => 2])),
            'result' => new SharedmobilitychResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        SharedmobilitychMakeResponse::call($ctx);
    ...
    {
        $preset = new SharedmobilitychResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new SharedmobilitychSpec([]),
            'response' => new SharedmobilitychResponse([]),
            'result' => new SharedmobilitychResult([]),
        ...
        [$got, $err] = SharedmobilitychMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new SharedmobilitychResult([])]);
        [, $err] = SharedmobilitychMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new SharedmobilitychSpec([]), 'result' => null]);
        [, $err] = SharedmobilitychMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SharedmobilitychSpec(['step' => 's']),
            'result' => new SharedmobilitychResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new SharedmobilitychOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SharedmobilitychMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new SharedmobilitychSpec(['step' => 's']),
            'result' => new SharedmobilitychResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new SharedmobilitychOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = SharedmobilitychMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new SharedmobilitychResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new SharedmobilitychSpec([]),
            'result' => new SharedmobilitychResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = SharedmobilitychMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SharedmobilitychMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new SharedmobilitychError('boom', 'boom');
        $utility->fetcher = function (SharedmobilitychContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SharedmobilitychSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = SharedmobilitychMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (SharedmobilitychContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = SharedmobilitychMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(SharedmobilitychResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        SharedmobilitychMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (SharedmobilitychContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new SharedmobilitychSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = SharedmobilitychMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new SharedmobilitychResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new SharedmobilitychSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = SharedmobilitychMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = SharedmobilitychMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = SharedmobilitychMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new SharedmobilitychResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, SharedmobilitychDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new SharedmobilitychResult(['ok' => false])]);
        $this->expectException(SharedmobilitychError::class);
        SharedmobilitychDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new SharedmobilitychResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', SharedmobilitychMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        SharedmobilitychMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new SharedmobilitychBaseFeature();
        $b = new SharedmobilitychBaseFeature();
        SharedmobilitychFeatureAdd::call($ctx, $a);
        SharedmobilitychFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): SharedmobilitychBaseFeature
    {
        $f = new SharedmobilitychBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        SharedmobilitychFeatureAdd::call($ctx, self::named_feature('a'));
        SharedmobilitychFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        SharedmobilitychFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        SharedmobilitychFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        SharedmobilitychFeatureAdd::call($ctx, $replace);
        ...
        SharedmobilitychFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): SharedmobilitychContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new SharedmobilitychSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = SharedmobilitychPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        SharedmobilitychPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new SharedmobilitychResponse(['headers' => null]),
            'result' => new SharedmobilitychResult([]),
        ]);
        SharedmobilitychResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        SharedmobilitychResultBody::call($ctx);

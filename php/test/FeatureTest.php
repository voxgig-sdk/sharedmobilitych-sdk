...
// Sharedmobilitych SDK feature test
//
...
class FtCtrl extends SharedmobilitychControl
{
...
class FtClient
{
    ...
    private SharedmobilitychUtility $utility;

    public function __construct(SharedmobilitychUtility $utility, array $options)
    {
    ...
    public function get_utility(): SharedmobilitychUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (SharedmobilitychContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new SharedmobilitychUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (SharedmobilitychContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = SharedmobilitychConfig::make_feature($name);
    ...
    private static function buildUrl(SharedmobilitychSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof SharedmobilitychError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new SharedmobilitychResponse($fetched);
        }
    ...
    private function fail(SharedmobilitychContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (SharedmobilitychContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

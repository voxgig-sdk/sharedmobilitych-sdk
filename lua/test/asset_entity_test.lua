...
describe("AssetEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SHAREDMOBILITYCH_TEST_ASSET_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local asset_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.asset")))
    local asset_ref01_data = nil
    if #asset_ref01_data_raw > 0 then
      asset_ref01_data = helpers.to_map(asset_ref01_data_raw[1][2])
    end
...
function asset_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("SHAREDMOBILITYCH_TEST_ASSET_ENTID")

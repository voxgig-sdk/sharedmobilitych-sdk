...
require("sharedmobilitych_types")
...
local SharedmobilitychSDK = {}
SharedmobilitychSDK.__index = SharedmobilitychSDK
...
SharedmobilitychSDK._make_feature = _make_feature


function SharedmobilitychSDK.new(options)
  local self = setmetatable({}, SharedmobilitychSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function SharedmobilitychSDK:options_map()
...
function SharedmobilitychSDK:get_utility()
  return Utility.copy(self._utility)
end


function SharedmobilitychSDK:get_root_ctx()
  return self._rootctx
end


function SharedmobilitychSDK:prepare(fetchargs)
...
function SharedmobilitychSDK:direct(fetchargs)
...
function SharedmobilitychSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function SharedmobilitychSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "SharedmobilitychSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function SharedmobilitychSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function SharedmobilitychSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "SharedmobilitychSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function SharedmobilitychSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = SharedmobilitychSDK.new(sdkopts)
  sdk.mode = "test"

...
local RegionEntity = {}
RegionEntity.__index = RegionEntity


function RegionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RegionEntity)
  self._name = "region"
  self._client = client
...
function RegionEntity:get_name()
  return self._name
end


function RegionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RegionEntity.new(self._client, opts)
end
...
function RegionEntity:mark_deleted()
  self._deleted = true
end


function RegionEntity:deleted()
  return true == self._deleted
end


function RegionEntity:data_set(args)
  if args ~= nil then
...
function RegionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RegionEntity:match_set(args)
  if args ~= nil then
...
function RegionEntity:match_get()
...
function RegionEntity:stream(action, args, callopts)
...
function RegionEntity:_run_op(ctx, post_done)
  local utility = self._utility

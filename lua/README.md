# Sharedmobilitych Lua SDK

The Lua SDK for the Sharedmobilitych API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Asset(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Sharedmobilitych(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── sharedmobilitych_sdk.lua    -- Main SDK module
...
The main module (`sharedmobilitych_sdk`) exports the SDK constructor

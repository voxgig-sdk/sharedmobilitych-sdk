# sharedmobilitych-mcp

[MCP](https://modelcontextprotocol.io) server exposing the Sharedmobilitych SDK as
two agent tools — `sharedmobilitych_list` and `sharedmobilitych_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/sharedmobilitych-mcp)
make build

# 2. Provide credentials via the environment
export SHAREDMOBILITYCH_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user sharedmobilitych \
  -- /absolute/path/to/sharedmobilitych-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./sharedmobilitych-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// sharedmobilitych_list: first page of records
{ "entity": "attribute" }
{ "entity": "attribute", "query": { } }

// sharedmobilitych_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/sharedmobilitych-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user sharedmobilitych \
     -- "$PWD"/dist/*/sharedmobilitych-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `sharedmobilitych_list` and `sharedmobilitych_load` tools now appear
   in new sessions. Ask the agent to *"list attribute using sharedmobilitych"*
   and it calls `sharedmobilitych_list` with `{"entity":"attribute"}`.
...
```sh
export SHAREDMOBILITYCH_APIKEY=sk_live_xxx            # API key
export SHAREDMOBILITYCH_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `sharedmobilitych_list` tool
...
### Call the `sharedmobilitych_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `sharedmobilitych_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `sharedmobilitych_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SHAREDMOBILITYCH_APIKEY` | API key sent with every request. |
| `SHAREDMOBILITYCH_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

asset | attribute | provider | region | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./sharedmobilitych-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"sharedmobilitych_load","arguments":{"entity":"asset","query":{"id":1}}}}'
```
